...
impl NamimadoService {
    ...
    #[cfg(feature = "browser-core")]
    pub fn tab_group_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .tab_group_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn tab_group_for(&self, host: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .tab_group_for(host)
    ...
    #[cfg(feature = "browser-core")]
    pub fn tab_hibernate_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .tab_hibernate_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn tab_hibernate_for(&self, host: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .tab_hibernate_for(host)
            .and_then(|s| serde_json::to_value(&s).ok())
    }

    #[cfg(feature = "browser-core")]
    pub fn tab_preview_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .tab_preview_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn tab_preview_for(&self, host: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .tab_preview_for(host)
            .and_then(|s| serde_json::to_value(&s).ok())
    }

    #[cfg(not(feature = "browser-core"))]
    pub fn tab_group_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn tab_group_for(&self, _h: &str) -> Option<serde_json::Value> { None }
    #[cfg(not(feature = "browser-core"))]
    pub fn tab_hibernate_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn tab_hibernate_for(&self, _h: &str) -> Option<serde_json::Value> { None }
    #[cfg(not(feature = "browser-core"))]
    pub fn tab_preview_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    ...
    #[cfg(feature = "browser-core")]
    pub fn fingerprint_randomize_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .fingerprint_randomize_list()
            .into_iter()
            .filter_map(|s| serde_json::to_value(&s).ok())
            .collect()
    }
    ...
    #[cfg(feature = "browser-core")]
    pub fn fingerprint_randomize_for(&self, host: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .fingerprint_randomize_for(host)
            .and_then(|s| serde_json::to_value(&s).ok())
    }

    #[cfg(feature = "browser-core")]
    pub fn cookie_jar_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .cookie_jar_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn cookie_jar_for(&self, host: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .cookie_jar_for(host)
    ...
    #[cfg(feature = "browser-core")]
    pub fn webgpu_policy_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .webgpu_policy_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn webgpu_policy_for(&self, host: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .webgpu_policy_for(host)
            .and_then(|s| serde_json::to_value(&s).ok())
    }

    #[cfg(not(feature = "browser-core"))]
    pub fn fingerprint_randomize_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn fingerprint_randomize_for(&self, _h: &str) -> Option<serde_json::Value> { None }
    #[cfg(not(feature = "browser-core"))]
    pub fn cookie_jar_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn cookie_jar_for(&self, _h: &str) -> Option<serde_json::Value> { None }
    #[cfg(not(feature = "browser-core"))]
    pub fn webgpu_policy_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    ...
    #[cfg(feature = "browser-core")]
    pub fn storage_quota_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .storage_quota_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn storage_quota_for(&self, host: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .storage_quota_for(host)
            .and_then(|s| serde_json::to_value(&s).ok())
    }

    #[cfg(feature = "browser-core")]
    pub fn clear_site_data_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .clear_site_data_list()
            .into_iter()
            .filter_map(|s| serde_json::to_value(&s).ok())
            .collect()
    }
    ...
    #[cfg(feature = "browser-core")]
    pub fn clear_site_data_applicable(&self, host: &str) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .clear_site_data_applicable(host)
            .into_iter()
            .filter_map(|s| serde_json::to_value(&s).ok())
            .collect()
    }
    ...
    #[cfg(not(feature = "browser-core"))]
    pub fn storage_quota_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn storage_quota_for(&self, _h: &str) -> Option<serde_json::Value> { None }
    #[cfg(not(feature = "browser-core"))]
    pub fn clear_site_data_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    ...
    #[cfg(feature = "browser-core")]
    pub fn cookie_banner_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .cookie_banner_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn cookie_banner_for(&self, host: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .cookie_banner_for(host)
            .and_then(|s| serde_json::to_value(&s).ok())
    }
    ...
    pub fn cookie_banner_hide_css(&self, host: &str) -> Option<String> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner.pipeline.cookie_banner_hide_css(host)
    }

    #[cfg(not(feature = "browser-core"))]
    pub fn cookie_banner_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn cookie_banner_for(&self, _h: &str) -> Option<serde_json::Value> { None }
    #[cfg(not(feature = "browser-core"))]
    ...
    #[cfg(feature = "browser-core")]
    pub fn inspector_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .inspector_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn inspector_visible(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .inspector_visible()
    ...
    #[cfg(feature = "browser-core")]
    pub fn inspector_get(&self, name: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .inspector_get(name)
    ...
    #[cfg(feature = "browser-core")]
    pub fn profiler_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .profiler_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn profiler_get(&self, name: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .profiler_get(name)
    ...
    #[cfg(feature = "browser-core")]
    pub fn console_rule_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .console_rule_list()
    ...
    #[cfg(not(feature = "browser-core"))]
    pub fn inspector_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn inspector_visible(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn inspector_get(&self, _n: &str) -> Option<serde_json::Value> { None }
    #[cfg(not(feature = "browser-core"))]
    pub fn profiler_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn profiler_get(&self, _n: &str) -> Option<serde_json::Value> { None }
    #[cfg(not(feature = "browser-core"))]
    pub fn console_rule_list(&self) -> Vec<serde_json::Value> { Vec::new() }

    // ── Media pack ───────────────────────────────────────────────

    #[cfg(feature = "browser-core")]
    pub fn media_session_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .media_session_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn media_session_for(&self, host: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .media_session_for(host)
            .and_then(|s| serde_json::to_value(&s).ok())
    }

    #[cfg(feature = "browser-core")]
    pub fn cast_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .cast_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn cast_applicable(&self, host: &str) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .cast_applicable(host)
            .into_iter()
            .filter_map(|s| serde_json::to_value(&s).ok())
            .collect()
    }
    ...
    #[cfg(feature = "browser-core")]
    pub fn subtitle_list(&self) -> Vec<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .subtitle_list()
    ...
    #[cfg(feature = "browser-core")]
    pub fn subtitle_for(&self, host: &str) -> Option<serde_json::Value> {
        let inner = self.inner.lock().expect("service mutex poisoned");
        inner
            .pipeline
            .subtitle_for(host)
    ...
    #[cfg(not(feature = "browser-core"))]
    pub fn media_session_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn media_session_for(&self, _h: &str) -> Option<serde_json::Value> { None }
    #[cfg(not(feature = "browser-core"))]
    pub fn cast_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn cast_applicable(&self, _h: &str) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn subtitle_list(&self) -> Vec<serde_json::Value> { Vec::new() }
    #[cfg(not(feature = "browser-core"))]
    pub fn subtitle_for(&self, _h: &str) -> Option<serde_json::Value> { None }
...
mod tests {
    ...
    #[test]
    fn dev_pack_default_profiler_auto_loads() {
        let svc = NamimadoService::new();
        // Inspectors + console-rules start empty (no auto-default);
        // profiler auto-registers a default profile.
    ...
    #[test]
    fn cookie_banner_default_rejects_all_with_hide_css() {
        let svc = NamimadoService::new();
        assert!(!svc.cookie_banner_list().is_empty());
        assert!(svc.cookie_banner_for("example.com").is_some());
        let css = svc.cookie_banner_hide_css("example.com").unwrap();
    ...
    #[test]
    fn storage_pack_defaults_wire_through() {
        let svc = NamimadoService::new();
        assert!(!svc.storage_quota_list().is_empty());
        assert!(!svc.clear_site_data_list().is_empty());
        assert!(svc.storage_quota_for("example.com").is_some());
        // Default applies to all hosts.
        assert!(!svc.clear_site_data_applicable("example.com").is_empty());
    }
    ...
    #[test]
    fn privacy_v2_pack_auto_registers_all_three_defaults() {
        let svc = NamimadoService::new();
        assert!(!svc.fingerprint_randomize_list().is_empty());
        assert!(!svc.cookie_jar_list().is_empty());
        assert!(!svc.webgpu_policy_list().is_empty());
        assert!(svc.fingerprint_randomize_for("example.com").is_some());
        assert!(svc.cookie_jar_for("example.com").is_some());
        assert!(svc.webgpu_policy_for("example.com").is_some());
    }
    ...
    #[test]
    fn tabs_pack_auto_registers_defaults() {
        let svc = NamimadoService::new();
        assert!(!svc.tab_group_list().is_empty());
        assert!(!svc.tab_hibernate_list().is_empty());
        assert!(!svc.tab_preview_list().is_empty());
        // Default hibernate / preview match any host; default group
        // has no host glob so it won't auto-match.
        assert!(svc.tab_hibernate_for("example.com").is_some());
        assert!(svc.tab_preview_for("example.com").is_some());
    }
    ...
    #[test]
    fn media_pack_defaults_auto_load() {
        let svc = NamimadoService::new();
        // Defaults auto-register so lists are non-empty.
        assert!(!svc.media_session_list().is_empty());
        assert!(!svc.cast_list().is_empty());
        assert!(!svc.subtitle_list().is_empty());
        assert!(svc.media_session_for("example.com").is_some());
        assert!(!svc.cast_applicable("example.com").is_empty());
        assert!(svc.subtitle_for("example.com").is_some());
    }
